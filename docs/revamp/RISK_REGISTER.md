...
| R-004 | Local LLM quality regressions hidden by visual polish | High | Golden-set score drop or confidence instability | Use operator-tunable coaching thresholds + draft-level drill-down examples + operator scorecard weekly reviews to catch regressions early; add golden-set eval harness in Phase 5 | Force fallback model profile and disable risky prompt path | LLM Runtime Owner |
