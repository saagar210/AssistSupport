...
| R-004 | Local LLM quality regressions hidden by visual polish | High | Golden-set score drop or confidence instability | Use operator-tunable coaching thresholds + draft-level drill-down examples + operator scorecard weekly reviews; enforce `check:llm-golden-set` policy gate and live-eval refresh in Phase 5 | Force fallback model profile and disable risky prompt path | LLM Runtime Owner |
